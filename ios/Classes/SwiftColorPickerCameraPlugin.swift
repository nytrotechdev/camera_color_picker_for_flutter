...
class CameraView: UIViewController, UINavigationControllerDelegate {
    ...
    @objc func appComesForeground(){
        print("App Comes ForeGround Observer ")
...
//        self.AVPreviewLayer.frame = self.previewLayer.bounds
    ...
    func addTopBar () {
        ...
        print("Difference Between HEight \(height - plHeight) and lHeight == \(plHeight)")
        let dif = 0
        let topGap = height*0.03
        
        self._viewTopBar = {
            let v                   = UIView(frame: CGRect(x: width*0.05 , y: topGap , width: width - width*0.10 , height: height*0.25 ) )
...
//            btn.setImage(_resizedImage, for: .normal)
        ...
        if(_picImageTarget == nil ){
            self._picImageTarget = {
                print("PIC Image Target -- at height point => \(pvHeight/2) width point => \(pvWidth/2) ----- MAIN SCREEN ... height -> \(pvHeight) ,,, \(pvWidth)")
                let _y          = (Int(height)/2)+dif-15
                let iv          = UIImageView(frame: CGRect(x: Int(pvWidth/2)-10, y:Int(pvHeight/2)-10  , width: 20, height: 20 ) )
                iv.image?.withRenderingMode(.alwaysTemplate)
                iv.image        = UIImage(named: "ic_target")
        ...
        self._imageTarget = {
            ...
            let iv          = UIImageView(frame: CGRect(x: Int(plWidth/2)-10, y:Int(plHeight/2)-10  , width: 20, height: 20 ) )
            iv.image?.withRenderingMode(.alwaysTemplate)
            iv.image        = UIImage(named: "ic_target")
            iv.tintColor    = UIColor.white
            iv.tintColor    = UIColor.black
...
//            btn.setImage(resized, for: .normal )
        ...
        self._labelBackTitle = {
            let l          = UILabel(frame: CGRect(x: width*0.10+40 , y: height*0.02+topGap+8, width: 80, height: 22 ) )
            l.adjustsFontSizeToFitWidth = true
            l.font         = UIFont(name: "Comic Sans MS", size:22)
            //            l.font         = l.font.withSize(42)
            l.textColor    = UIColor.black
            l.text         = "Color Grading"
            return l
        }()
...
//        self.view.bringSubviewToFront( cvSlider )
    ...
    @IBAction func sliderTip(_ sender : UISlider){
        ...
        print("Slider Taped For Value -> \(sender.value) ")
    ...
    func addBottomButtons(){
        ...
        let height  = self.view.frame.height
        self._btnScreenshot = {
            let btn = UIButton(frame: CGRect(x: (width*0.50)-30 ,  y: (height*0.90)-30, width: 60, height: 60 ) )
            btn.addTarget(self, action: #selector(btnCapturePhoto), for: .touchUpInside)
            btn.setImage( UIImage(named: "ic_capture"), for: .normal)
            return btn
        }()
...
//
    ...
    @objc func finalizeScreenshot(){
        ...
        showingPic                     = false
        print("Print Finalize ScreenShot...")
        takeScreenshot()
...
extension CameraView : AVCaptureVideoDataOutputSampleBufferDelegate , AVCapturePhotoCaptureDelegate {
    ...
    @objc func pinchToZoom(_ sender: UIPinchGestureRecognizer) {
        ...
        print("PINCH TO ZOOM  => \(sender.velocity)")
...
//                detectionOverlay.addSublayer(shapeLayer)
    ...
    func setupLayers() {
        ...
        print(" __ SETUP DETECTION LAYER -- ")
    ...
    func createRoundedRectLayerWithBounds(_ bounds: CGRect) -> CALayer {
        ...
        print("CREATE RECT LAYER -- WITH BOUNDS \(bounds)")
...
//            }
            ...
            print("Slider cell \(indexPath.row)")
            ...
            let sliderCell = collectionView.dequeueReusableCell(withReuseIdentifier: SliderCell.identifier , for: indexPath) as! SliderCell
            let _imageName = sliderImages[indexPath.row]
            print("Slider cell DIAMONDS \(indexPath.row)")
